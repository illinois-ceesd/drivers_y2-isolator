...
printf "Isolator testing suite running on Quartz:\n"
...
./test-serial-3d-combustion-quartz.sh ${EMIRGE_HOME} ${TESTING_LOG_FILE} ${TESTING_RESULTS_FILE} "serial-3d-combustion"

# Next - do parallel isolator tests (waits on parallel capability)
# {2,4} GPUs
printf "Testing parallel injection 3D with combustion... \n"
./test-parallel-3d-combustion-quartz.sh ${EMIRGE_HOME} ${TESTING_LOG_FILE} ${TESTING_RESULTS_FILE} "parallel-3d-combustion"
